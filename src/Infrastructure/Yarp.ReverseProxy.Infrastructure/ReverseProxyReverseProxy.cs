...
public class ReverseProxyReverseProxy : IReverseProxy
{
    private ReverseProxyReloadToken _reloadToken = new ReverseProxyReloadToken();
    private IServiceProvider _sp;
    private IMemoryCache _cache;
    private ILogger Logger;
    public event ConfigChangeHandler ChangeConfig;

    public ReverseProxyReverseProxy(IServiceProvider sp, IMemoryCache cache, ILoggerFactory loggerFactory)
    {
        Logger = loggerFactory.CreateLogger<ReverseProxyReverseProxy>();
        _sp = sp;
        _cache = cache;
    ...
    public IProxyConfig GetConfig()
    {
        ...
        var exist = _cache.TryGetValue<IProxyConfig>("ReverseProxyConfig", out var config);
        if (exist)
        {
            return config;
        }
    ...
    public void ReloadConfig()
    {
        ...
        SetConfig();
        Interlocked.Exchange<ReverseProxyReloadToken>(ref _reloadToken,
            new ReverseProxyReloadToken()).OnReload();
    }
    ...
    {
        var DbContext = _sp.CreateScope().ServiceProvider.GetRequiredService<ReverseProxyDbContext>();
    ...
    private static IReadOnlyList<Yarp.ReverseProxy.Configuration.RouteHeader>? CreateRouteHeaders(List<Entity.RouteHeader> routeHeaders)
    {
    ...
    private static Yarp.ReverseProxy.Configuration.RouteHeader CreateRouteHeader(Entity.RouteHeader routeHeader)
    {
    ...
    private static IReadOnlyList<Configuration.RouteQueryParameter>? CreateRouteQueryParameters(IReadOnlyList<Entity.RouteQueryParameter> routeQueryParameters)
    {
    ...
    private static Configuration.RouteQueryParameter CreateRouteQueryParameter(Entity.RouteQueryParameter routeQueryParameter)
    {
        return new Configuration.RouteQueryParameter()
        {
    ...
    private static Yarp.ReverseProxy.Configuration.SessionAffinityConfig? CreateSessionAffinityOptions(Entity.SessionAffinityConfig sessionAffinityOptions)
    {
    ...
    private static Yarp.ReverseProxy.Configuration.SessionAffinityCookieConfig? CreateSessionAffinityCookieConfig(Entity.SessionAffinityCookie sessionAffinityCookie)
    {
    ...
    private static Yarp.ReverseProxy.Configuration.HttpClientConfig? CreateHttpClientConfig(Entity.HttpClientConfig proxyHttpClientOptions)
    {
        ...
        Configuration.WebProxyConfig? webProxy;
        var webProxySection = proxyHttpClientOptions.WebProxy;
        if (webProxySection != null)
        {
            webProxy = new Configuration.WebProxyConfig()
            {
